...
export function ProfileForm() {
  ...
  const visibleImage = shouldRemoveImage ? null : imagePreview ?? user?.image ?? null;
  const {
    formState: { errors },
    handleSubmit,
    register,
    ...
    resolver: zodResolver(createProfileSchema(isLearner)),
  });
  ...
  function handleImageChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!profileImageAccept.includes(image.type as (typeof profileImageAccept)[number])) {
  ...
  return (
    <form
      ...
      noValidate
    >
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5 sm:p-7">
        {isIncompleteLearner ? (
      ...
      <aside className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5 sm:p-6">
        <div className="flex items-center gap-4">
          <ProfileAvatar image={visibleImage} name={user?.name} email={user?.email} />
        ...
        <p className="mt-4 text-sm leading-6 text-slate-500">
          Optional JPEG, PNG, or WebP picture up to 2 MB.
        </p>
        {selectedImage ? (
          <p className="mt-3 truncate text-sm font-semibold text-emerald-700">
            Previewing {selectedImage.name}
          </p>
        ) : null}
        {shouldRemoveImage ? (
          <p className="mt-3 text-sm font-semibold text-rose-700">
            Picture will be removed after saving is connected.
          </p>
        ) : null}
...
function getDefaultValues(user: Session["user"] | undefined) {
  return {
    city: user?.city ?? "",
    email: user?.email ?? "",
    learnerType: user?.learnerType?.toLowerCase() as ProfileFormValues["learnerType"],
    mobile: user?.mobile ?? "",
    name: user?.name ?? "",
  };
}

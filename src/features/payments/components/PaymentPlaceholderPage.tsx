...
type PaymentPlaceholderPageProps = {
  paymentId: string;
};

export async function PaymentPlaceholderPage({
  paymentId,
}: PaymentPlaceholderPageProps) {
  ...
  const isCoursePayment = payment.purpose === "COURSE";
  const backHref = isCoursePayment && payment.course
      ...
      <Container>
        <section className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            ...
            <PaymentDetail label="Purpose" value={formatStatus(payment.purpose)} />
            {payment.course ? (
              <PaymentDetail label="Course" value={payment.course.title} />
            ) : null}
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <ButtonLink href={backHref}>
              {isCoursePayment ? "Back to Course" : "Back to Donate"}
